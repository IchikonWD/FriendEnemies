...
const alertUser = function(fromUser, toId, type, postContent) {
    return new Promise(function(resolve, reject) {
        ...
        switch(type) {
            case "new_friend":
                alert.alert_text = `${alert.from_name} ha aceptado tu petición de amistad.`;
                break;
            case "liked_post":
                alert.alert_text = `${alert.from_name} le ha dado like a tu post, '${postContent}'.`;
                break;
            case "commented_post":
                alert.alert_text = `${alert.from_name} ha comentado tu post, '${postContent}'.`;
                break;
            default: return reject("No hay un tipo válido de alerta.");
        }
    ...
    user.save((err, newUser) => {
        if(err) {
            if(err.errmsg && err.errmsg.includes("duplicate key error") && err.errmsg.includes("email")) {
                return res.json({ message: "El Emmail proporcionado ya esta registrado."} );
            }
            return res.json({ message: "Algo ha ido mal." });
        } else {
...
const generateFeed = function({ payload }, res) {
    ...
    let myPosts = new Promise(function(resolve, reject) {
        User.findById(payload._id, "", {lean: true}, (err, user) => {
            ...
            if(!user) { return res.json(404, { message: "El Usuario no existe." }); }
...
const getSearchResults = function({ query, payload }, res) {
    if(!query.query) { return res.json({ err: "Falta una query." }); }
    User.find({ name: { $regex: query.query, $options: "i" } }, "name profile_image friends friend_requests", (err, results) => {
        ...
        return res.status(200).json({ message: "Obteniendo resultados de búsqueda", results: results });
    });
}
...
const makeFriendRequest = function({params}, res) {
    User.findById(params.to, (err, user) => {
        ...
        if(containsDuplicate([params.from, ...user.friend_requests])) {
            return res.json({ message: "La solicitud de amistad ya ha sido enviada." });
        }
        
        user.friend_requests.push(params.from);
        user.save((err, user) => {
            if(err) { return res.json({ err: err }); }
            return res.statusJson(201, { message: "Se ha enviado correctamente la solicitud de amistad." });
        });
    });
}
...
const getFriendRequests = function({query}, res) {
    ...
    User.find({ '_id': { $in: friendRequests } }, "name profile_image", (err, users) => {
        if(err) { return res.json({ err: err }); }
        return res.statusJson(200, { message: "Obteniendo resultados de búsqueda", users: users });
    });
}
        ...
        let promise = new Promise(function(resolve, reject) {
            ...
            if(query.resolution == "accept") {
                ...
                if(containsDuplicate([params.from, ...user.friends])) {
                    return res.json({ message: "Error Duplicado." });
                }
        ...
        promise.then(() => {
            user.save((err, user) => {
                ...
                alertUser(user, params.from, "new_friend").then(() => {
                    res.statusJson(201, { message: "Solicitud de amistad resuelta" });
                });
            });
        })
    });
...
const createPost = function({ body, payload }, res) {
    if(!body.content || !body.theme) {
        return res.statusJson(400, { message: "Datos insuficientes enviados con la solicitud." });
    }
    ...
    User.findById(userId, (err, user) => {
        ...
        user.save((err) => {
            if(err) { return res.json({ err: err }); }
            return res.statusJson(201, { message: "Post creado", newPost: newPost });
        });
    });
}
...
const likeUnlike = function({ payload, params }, res) {
    User.findById(params.ownerid, (err, user) => {
        ...
        promise.then(() => {
            user.save((err, user) => {
                if(err) { return res.json({ err: err }); }
                res.statusJson(201, { message: "Me gusta o No me gusta una publicación..." });
            });
        });
    });
}
...
const sendMessage = function({ body, payload, params }, res) {
    ...
    sendMessagePromise.then(() => {
        return res.statusJson(201, { message: "Enviando Mensaje" });
    });
}
...
const resetMessageNotifications = function({ payload }, res) {
    User.findById(payload._id, (err, user) => {
        ...
        user.save((err) => {
            if(err) { return res.json({ err: err }); }
            return res.statusJson(201, { message: "Restablecer notificaciones de mensajes." });
        });
    });
}
...
const deleteMessage = function({ payload, params }, res) {
    User.findById(payload._id, (err, user) => {
        ...
        user.save((err) => {
            if(err) { return res.json({ err: err }); }
            return res.statusJson(201, { message: "Mensaje eliminado" });
        });
    });
}
...
const bestieEnemyToggle = function({ payload, params, query }, res) {
    ...
    if(toggle != "besties" && toggle != "enemies") {
        return res.json({ message: "Consulta incorrecta proporcionada" });
    }
    ...
    User.findById(myId, (err, user) => {
        ...
        if(!user.friends.includes(friendId)) {
            return res.json({ message: "No eres amigo de este usuario." });
        }
        ...
        if(arr.includes(friendId)) {
            ...
            if(toggle == "besties" && user.besties.length >= 2) {
                return res.json({ message: "Tienes la máxima cantidad de mejores amigos." });
            }
            arr.push(friendId);
        }
