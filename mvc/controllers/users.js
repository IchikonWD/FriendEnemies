  ...
  return new Promise(function (resolve, reject) {
    ...
    switch (type) {
      ...
      case "commented_post":
        ...
        break;
      default:
        return reject("No hay un tipo válido de alerta.");
    }
  ...
  user.save((err, newUser) => {
    if (err) {
      if (
        ...
        return res.json({
          message: "El Emmail proporcionado ya esta registrado.",
        });
      }
...
const generateFeed = function ({ payload }, res) {
  ...
  let myPosts = new Promise(function (resolve, reject) {
    User.findById(payload._id, "", { lean: true }, (err, user) => {
      ...
      if (!user) {
        return res.json(404, { message: "El Usuario no existe." });
      }
...
const getSearchResults = function ({ query, payload }, res) {
  if (!query.query) {
    return res.json({ err: "Falta una query." });
  }
  User.find(
    ...
    (err, results) => {
      ...
      return res
        .status(200)
        .json({
          message: "Obteniendo resultados de búsqueda",
          results: results,
        });
    }
  );
  ...
  User.findById(params.to, (err, user) => {
    ...
    user.save((err, user) => {
      ...
      return res.statusJson(201, {
        message: "Se ha enviado correctamente la solicitud de amistad.",
      });
    });
  });
};
...
const createPost = function ({ body, payload }, res) {
  if (!body.content || !body.theme) {
    return res.statusJson(400, {
      message: "Datos insuficientes enviados con la solicitud.",
    });
  }
  ...
  User.findById(params.ownerid, (err, user) => {
    ...
    promise.then(() => {
      user.save((err, user) => {
        ...
        res.statusJson(201, {
          message: "Me gusta o No me gusta una publicación...",
        });
      });
    });
  });
...
const resetMessageNotifications = function ({ payload }, res) {
  User.findById(payload._id, (err, user) => {
    ...
    user.save((err) => {
      ...
      return res.statusJson(201, {
        message: "Restablecer notificaciones de mensajes.",
      });
    });
  });
};
  ...
  User.findById(myId, (err, user) => {
    ...
    if (arr.includes(friendId)) {
      ...
      if (toggle == "besties" && user.besties.length >= 2) {
        return res.json({
          message: "Tienes la máxima cantidad de mejores amigos.",
        });
      }
      arr.push(friendId);
    }
